...
impl WithComputeUnit for Vec<Instruction> {
    fn with_compute_unit(mut self, unit: Option<u32>) -> Self {
        if let Some(compute_unit) = unit {
            self.insert(
                0,
                ComputeBudgetInstruction::set_compute_unit_limit(compute_unit),
            );
        }
        self
    }
